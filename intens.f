...
C     IMPLICIT LOGICAL (A-Z)
     ...
     2        NotateLevelC(MaxNumLevelC,11),Nu0
     ...
     1        J_i,J_f
      ...
      INTEGER LeftTime,J2MIN,J2MAX
             
      INTEGER IntJ_i,IntJ_f

      REAL*8 ThreeJSym,SexJSym,EnerSource

      INTEGER Date1,Hour1,Min1,Sec1,Date2,Hour2,Min2,Sec2
...
3003  FORMAT(1H0,'                 CALCUL.  ROT. MATR. ELEMENTS '/)
      CALL Calc_RotateMatrElement(ThreeJsymbol,SexJsymbol)
C                                                                          C
C**************************************************************************C
...
C**************************************************************************C
C                                                                          C
C     PREREAD EIGEVECTORS UNCONTRACTED COEFFICIENTS 
C     FOR LOWER A AND B  STATES AND WRITE TO THE TEMP. FILE 49
C                                                                          C
C**************************************************************************C
C                                                                          C
      WRITE(6,3004)
3004  FORMAT(1H0,'                 PREREAD EIGEVECTORS UNCONTRACTED ',
     1                             'COEFFICIENTS'/)
...
C                                                                          C
     ...
     1 .OR. (IndSym_i*IndSym_f.EQ.4 .AND. .NOT.SYMM) )THEN  ! -- A1 <- A2 or B1 <-B2 
...
C                                                                            C
       ...
       IntJ_i = INT(J_i)
       IntJ_f = INT(J_f)
       SexJSym = SexJsymbol(IntJ_i+1,N_AB+1,N_f-N_AB+2,IntJ_f-IntJ_i+2)
       
C                                                                            C
C****************************************************************************C
C                                                                            C
      IF ( SexJSym.NE.0.0 ) THEN  
...
C****************************************************************************C
         ...
         RotMatrElem = Minus*SexJSym*ThreeJSym
...
C***********************
        IF ( IntACT ) THEN    ! ### Absorption ### 
             AbsInt=8.0E-36*PI*PI*PI*AVOGNO*
     1              Nu_i_f*(1.0D+00-DEXP(Nu_i_f*TEMPCO))/
     1              (3.0D+00*Planck*VellGT)
         ELSE                  ! ### Emission ###  
             AbsInt=64.0E-36*PI*PI*PI*PI*AVOGNO*VellGT*
     1              DEXP( Nu0*TEMPCO )*
...
C                                                                            C
     ...
     3    EnergyC(LevelC),EnergyAB((LevelAB))


1014  FORMAT( 1X,F11.4,'  ',D12.4,'  ',D12.4,'  ',
     1        I2,' ',I2,' ',I2,' ',I2,' 'I2,' ',I2,' ',I2,' ',
     3        A2,' ',A2,'   ',
     1        I2,' ',I2,' ',I2,'  ',I2,'  ',I2,' ',A2,' ',
     1        I2,' ',I2,' ',I2,'  ',I2,'  ',I2,' ',A2,' ',
     1        I1,'  ',F11.4,'   ',F11.4)
...
C UPDATES     :
C LANGUAGE    : FORTRAN
C PURPOSE     : READ EIGEVECTORS UNCONTRACTED COEFFICIENTS 
...
C UPDATES     :
C LANGUAGE    : FORTRAN
C PURPOSE     : PREREAD EIGEVECTORS UNCONTRACTED COEFFICIENTS 
C             : FOR LOWER A and B STATES
C             : 
C SUBPROGRAMS :
C      CALLED :
